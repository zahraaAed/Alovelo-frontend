...
<Header/>
      {/* Hero Section */}
      <Image 
          src={`http://localhost:5000/images/${aboutData.hero_image.split("/").pop()}`} 
          alt="About Hero"
          width={1146}
          height={768}
      ...
      <section className={styles.visionMission}>
        ...
        <div className={styles.textContainer}>
          <div>
            ...
            <p>{aboutData?.vision_content || "Vision description goes here."}</p>
          </div>
          <div>
            <h3>{aboutData?.mission_title || "Our Mission"}</h3>
            <p>{aboutData?.mission_content || "Mission description goes here."}</p>
          </div>
        </div>
        <Image 
          src={`http://localhost:5000${aboutData?.main_image?.startsWith("/") ? aboutData.main_image : "/" + aboutData.main_image}`} 
          alt="Vision & Mission" 
          width={800} 
          height={500} 
          ...
          {aboutData?.features?.length > 0 ? (
            aboutData.features.map((feature, index) => (
              <div key={index} className={styles.featureItem}>
                <Image 
                  src={`http://localhost:5000${feature.icon?.startsWith("/") ? feature.icon : "/" + feature.icon}`} 
